/* ============================================================
   best_plan_finder_addon.js — Best Plan Finder v1
   build tag: bpf2 / intended app build LC-318

   Additive browser-side feature for My Pension Project.
   - Does not replace engine.js.
   - Does not save a saved model directly.
   - Finds earliest safe retirement using a fixed annuity amount.
   - Apply updates the current modelling rail and an in-memory annuity row,
     then the existing Save model feature can capture the resulting model without an immediate DB write.
...
(function (global) {
  ...
  function cloneForRun(src, opts) {
    ...
    out.annuities = (src.annuities || [])
      .filter(a => String(a.annuity_name || a.name || '') !== ANN_NAME)
      .concat([ann]);
  ...
  function evaluateRows(rows, opts) {
    ...
    const annIdx = monthIdx(opts.annuityDate);
    ...
    for (const r of monthly) {
      ...
      if (idx === annIdx) { annuityRow = r; annuityBuy += buyFor(r, opts.owner); }
      lastRow = r;
    }
    ...
    const annuityOK = opts.mustBuyAnnuity ? annuityBuy + 1 >= opts.annuityAmount : true;
    const pass = reserveOK && shortfallOK && annuityOK;
    let reason = 'Pass';
    if (!annuityOK) reason = 'Fixed annuity could not be bought at ' + dateLabel(opts.annuityDate) + ' without enough member pot.';
  ...
  function bestPlanFixedAnnuity(srcData, opts) {
    ...
    for (let idx = minIdx; idx <= maxIdx; idx++) {
      ...
      const cfg = opts.rebuildForDate(retireDate);
      const runData = cloneForRun(srcData, safeOpts);
  ...
  function buildCss() {
    ...
    st.textContent = `
      .bpf-panel{grid-column:1 / -1;margin:0;padding:14px;border:1px solid var(--line);border-radius:12px;background:linear-gradient(135deg,rgba(44,122,123,.10),rgba(44,122,123,.03));box-shadow:var(--shadow)}
      ...
      .bpf-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(176px,1fr));gap:10px}.bpf-field{background:var(--surface);border:1px solid var(--line);border-radius:10px;padding:10px}.bpf-field label{display:block;font-size:12px;font-weight:700;color:var(--ink-soft);margin-bottom:6px}.bpf-field input,.bpf-field select{width:100%;min-height:38px;padding:8px 9px;border:1px solid var(--line-strong);border-radius:8px;background:var(--surface-2);color:var(--ink);font-size:14px}.bpf-inline{display:flex;gap:8px;align-items:center}.bpf-chiprow{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}.bpf-chip{border:1px solid var(--accent-border);background:var(--accent-soft);color:var(--accent-dark);border-radius:999px;padding:6px 10px;font-size:12px;font-weight:700;cursor:pointer;min-height:32px}.bpf-switches{display:flex;flex-wrap:wrap;gap:9px;margin-top:10px}.bpf-tog{display:flex;align-items:center;gap:7px;font-size:13px;color:var(--ink);background:var(--surface);border:1px solid var(--line);border-radius:999px;padding:8px 11px}.bpf-actions{display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-top:12px}.bpf-btn{border:none;border-radius:10px;padding:10px 14px;font-weight:800;cursor:pointer;min-height:42px}.bpf-run{background:var(--accent);color:white}.bpf-apply{background:#27ae60;color:white}.bpf-discard{background:var(--surface);color:var(--ink);border:1px solid var(--line-strong)}.bpf-msg{font-size:13px;font-weight:700}.bpf-msg.err{color:#c0392b}.bpf-msg.ok{color:#2f8f5b}.bpf-result{margin-top:12px}.bpf-card{border-radius:12px;border:1px solid var(--line);background:var(--surface);padding:12px}.bpf-card.ok{border-color:#a7f3d0;background:#ecfdf5;color:#065f46}.bpf-card.bad{border-color:#fecaca;background:#fef2f2;color:#991b1b}.bpf-metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:8px;margin-top:10px}.bpf-metric{border:1px solid rgba(0,0,0,.08);border-radius:10px;padding:9px;background:rgba(255,255,255,.55)}.bpf-metric b{display:block;font-size:16px}.bpf-metric span{font-size:11px;opacity:.8}.bpf-dialwrap{display:flex;gap:10px;align-items:center}.bpf-dial{width:76px;height:76px;border-radius:50%;border:8px solid var(--accent-soft);background:var(--surface-2);position:relative;box-shadow:inset 0 0 0 1px var(--line);touch-action:none;flex:0 0 auto}.bpf-dial:after{content:'';position:absolute;left:50%;top:12px;width:4px;height:28px;margin-left:-2px;background:var(--accent);border-radius:3px;transform-origin:50% 26px;transform:rotate(var(--bpf-rot,-45deg))}.bpf-dial:before{content:'';position:absolute;left:50%;top:50%;width:12px;height:12px;margin:-6px 0 0 -6px;background:var(--accent);border-radius:50%}.bpf-ratebox{flex:1 1 auto}.bpf-note{font-size:11.5px;color:var(--ink-soft);line-height:1.35;margin-top:8px}
      @media (prefers-color-scheme: dark){.bpf-card.ok{background:rgba(6,95,70,.22);color:#a7f3d0}.bpf-card.bad{background:rgba(127,29,29,.24);color:#fecaca}.bpf-metric{background:rgba(255,255,255,.04)}}
  ...
  function panelHtml() {
    ...
    return `
      <div class="params bpf-panel" id="bpf_panel">
        <div class="bpf-head">
          <div><div class="bpf-title">Best Plan Finder</div><div class="bpf-sub">Find the earliest safe retirement date using a fixed annuity amount and protected pot reserve. Apply updates the live model only; use your existing Save model button afterwards.</div></div>
        ...
        <div class="bpf-switches">
          <label class="bpf-tog"><input type="checkbox" id="bpf_crashes" checked> Include crashes</label>
          <label class="bpf-tog"><input type="checkbox" id="bpf_shortfall" checked> No shortfall allowed</label>
          <label class="bpf-tog"><input type="checkbox" id="bpf_must_buy" checked> Must buy annuity</label>
        </div>
  ...
  function readBpfInputs() {
    ...
    return {
      ...
      includeCrashes: !!qs('bpf_crashes').checked,
      noShortfall: !!qs('bpf_shortfall').checked,
      mustBuyAnnuity: !!qs('bpf_must_buy').checked
    };
  }
  function runBestPlan() {
    const out = qs('bpf_result');
    try {
      setMsg('Running…', ''); out.innerHTML = ''; qs('bpf_apply').disabled = true; lastResult = null;
  ...
  function renderResult(res) {
    const out = qs('bpf_result');
    if (!res || !res.feasible) {
      out.innerHTML = '<div class="bpf-card bad"><strong>No feasible date found.</strong><div style="margin-top:6px;">' + esc(res && res.reason ? res.reason : 'No date in the selected range passed all rules.') + '</div></div>';
      return;
    }
    const d = res.detail || {};
    const annIncome = (res.opts.annuityAmount || 0) * ((res.opts.annuityRate || 0) / 100);
    out.innerHTML = '<div class="bpf-card ok"><strong>Plan passes.</strong> Earliest safe retirement is <strong>' + esc(dateLabel(res.earliestDate)) + '</strong>.' +
      ...
      '<div class="bpf-metric"><b>' + gbp(res.opts.annuityAmount) + '</b><span>Fixed annuity purchase</span></div>' +
      '<div class="bpf-metric"><b>' + gbp(annIncome) + '/yr</b><span>Estimated gross annuity income</span></div>' +
      '<div class="bpf-metric"><b>' + gbp(d.endPot) + '</b><span>End pot</span></div>' +
      '</div><div class="bpf-note">Apply updates the live modelling rail and the in-memory annuity row named “' + ANN_NAME + '”. It will not create/overwrite a saved model; use the existing Save model button afterwards.</div></div>';
  }

  function buildLiveAnnuityRow(opts) {
    return {
      annuity_name: ANN_NAME,
      member_name: ownerKeyToMember(opts.owner),
      purchase_date: toInputDate(opts.annuityDate),
      purchase_amount: opts.annuityAmount,
      annuity_rate: opts.annuityRate,
      escalation_pct: opts.escalationPct,
  ...
  function clearApplied() {
    try {
      if (global.__bpfAppliedAnnuity && typeof data !== 'undefined') {
        data.annuities = (data.annuities || []).filter(a => String(a.annuity_name || a.name || '') !== ANN_NAME);
      }
      global.__bpfAppliedAnnuity = false;
      global.__bpfAppliedAnnuityRow = null;
      global.__bpfForceSharedRetirement = false;
      if (typeof _pendingSaveState !== 'undefined') _pendingSaveState = null;
    } catch (e) { console.warn('Best Plan Finder clearApplied failed:', e); }
  }
  function applyBestPlan() {
    if (!lastResult || !lastResult.feasible) { setMsg('Run Best Plan Finder first.', 'err'); return; }
    try {
      setMsg('Applying…', '');
      ...
      // 2) Update the in-memory annuity collection. This is not written to Supabase here.
      const annRow = buildLiveAnnuityRow(res.opts);
      if (typeof data !== 'undefined') {
        data.annuities = (data.annuities || []).filter(a => String(a.annuity_name || a.name || '') !== ANN_NAME).concat([annRow]);
      }
      global.__bpfAppliedAnnuity = true;
      global.__bpfAppliedAnnuityRow = annRow;
      global.__bpfForceSharedRetirement = true;
      if (typeof _pendingSaveState !== 'undefined') _pendingSaveState = null;
      // 3) Redraw normal model charts. Existing Save model remains the save mechanism.
  ...
  function initUi() {
    ...
    const anchor = qs('optPair') || qs('modelRail') || document.querySelector('#Modelling .params') || document.querySelector('#Modelling');
    if (!anchor) return;
    buildCss();
    const wrap = document.createElement('div'); wrap.innerHTML = panelHtml();
    if (anchor.id === 'modelRail') {
    ...
    document.querySelectorAll('#bpf_panel .bpf-chip').forEach(btn => btn.addEventListener('click', () => {
      const p = String(btn.dataset.set || '').split(':'); const el = qs(p[0]); if (el) el.value = p[1];
    }));
  ...
  global.BestPlanFinder = Object.assign(global.BestPlanFinder || {}, { clearApplied: clearApplied, build: BUILD });
  function boot() {
    let tries = 0;
    const t = setInterval(() => {
      tries++;
